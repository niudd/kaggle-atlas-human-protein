...
def do_augmentation(image, mask=None):
    ...
    if np.random.rand() < 0.5:
        ...
        if c==0:
            ...
            if mask is not None:
                mask = do_vertical_flip(mask)
        elif c==2:
            k = np.random.randint(1, 4)
            image = do_4channels(image, do_rotation, k=k)
            if mask is not None:
                mask = do_rotation(mask, k)

    if np.random.rand() < 0.5:
        c = np.random.choice(3)
        if c==0:
            image = do_4channels(image, do_brightness_shift, alpha=np.random.uniform(-0.05, +0.05))
        elif c==1:
            image = do_4channels(image, do_brightness_multiply, alpha=np.random.uniform(1-0.05, 1+0.05))
        elif c==2:
            image = do_4channels(image, do_gamma, gamma=np.random.uniform(1-0.05, 1+0.05))
    ...
    if np.random.rand() < 0.5:
        ...
        if c==0:
            ...
            image = do_4channels(image, do_perspective_transform)
            if mask is not None:
                mask = do_perspective_transform(mask)
    if mask is not None:
        return image, mask.reshape(1, 512, 512)
    return image#.reshape(4, 512, 512)
