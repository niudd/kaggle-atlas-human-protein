...
from model_single_multi_label import AtlasResNet34, AtlasResNet18

from utils import save_checkpoint, load_checkpoint, set_logger
from dataset_single_multi_label import prepare_trainset
...
def run_check_net(train_dl, val_dl, multi_gpu=[0, 1]):
    ...
    #---
    #if MODEL == 'RESNET34':
    net = AtlasResNet34(debug=False, num_classes=2).cuda(device=device)
#     elif MODEL == 'RESNET18':
...
NUM_WORKERS = 20
warm_start, last_checkpoint_path = False, 'checkpoint/single_multi_label/%s_%s_v1_seed%s/best.pth.tar'%(MODEL, IMG_SIZE, SEED)
checkpoint_path = 'checkpoint/single_multi_label/%s_%s_v1_seed%s'%(MODEL, IMG_SIZE, SEED)
LOG_PATH = 'logging/single_multi_label/%s_%s_v1_seed%s.log'%(MODEL, IMG_SIZE, SEED)#
torch.cuda.manual_seed_all(SEED)

NUM_EPOCHS = 100
early_stopping_round = 10#500#50
LearningRate = 0.01#phase1: 0.02, phase2: 0.002

######### Load Atlas Protein data #########
# dataset
train_dl, val_dl = prepare_trainset(BATCH_SIZE, NUM_WORKERS, SEED, debug, IMG_SIZE)
